...
for seqfile in filelist:
    ...
    if (seqfile.find(".F_") != -1) and (seqfile.find(".ab1") != -1):
        print "current forward seq file: " + seqfile

        # get sample_id
        sample_id = seqfile[3:seqfile.find(".F_")]
        ...
        for record in SeqIO.parse(handle, "abi"):
            ...
            for seqfile2 in filelist:
                if (seqfile2.find(sample_id + ".R_") != -1) and (seqfile2.find(".ab1") != -1):
...
print "writing ETS.fasta"
SeqIO.write(ITS_records, "ETS.fasta", "fasta")
print "aligning ETS.fasta with clustalw"

stop_timer = Event()
timer = SimpleTimer(stop_timer)
timer.start()

clustalw_cline = ClustalwCommandline("clustalw", infile="ETS.fasta")
clustalw_cline()
